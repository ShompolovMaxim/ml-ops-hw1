...
async def train_model(model_type: str, dataset_path: str, hyperparams: Optional[Dict[str, Any]] = None):
    """Train new ML model from dataset."""
    try:
        if not dataset_path:
            raise ValueError("dataset_path required")
        
        logger.info(f"Starting training: model_type={model_type}, dataset={dataset_path}")
...
async def retrain_model(model_id: str, dataset_path: str, hyperparams: Optional[Dict[str, Any]] = None):
...
async def predict(model_id: str, data: Dict[str, Any] = None):
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Prediction timed out for model: {model_id} after {timeout}s")
            raise HTTPException(status_code=504, detail=f"Prediction timed out after {int(timeout)} seconds")
...
async def list_datasets():
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Failed to list datasets: {str(e)}")
...
async def delete_dataset(filename: str):
    ...
    try:
        ...
        try:
            ...
            subprocess.run(["git", "commit", "-m", f"remove dataset {filename}"], check=False)
        ...
        raise HTTPException(status_code=400, detail=f"Failed to delete dataset: {str(e)}")

...
class ModelManager:
    ...
    def train(self, model_type: str, X_train, y_train, X_test=None, y_test=None, 
              hyperparams: Optional[Dict] = None, log_to_clearml: bool = True) -> Dict:
        ...
        if log_to_clearml:
            task_id = self._log_to_clearml(model_id, model_type, hyperparams, metrics, model=model)
            if not task_id:
                raise RuntimeError("Failed to store model in ClearML; aborting to enforce ClearML-only storage")
    ...
    def retrain(self, model_id: str, X_train, y_train, X_test=None, y_test=None,
                hyperparams: Optional[Dict] = None, log_to_clearml: bool = True) -> Dict:
        ...
        if log_to_clearml:
            task_id = self._log_to_clearml(model_id, model_type, hyperparams, metrics, task_suffix="retrain", model=model)
            if not task_id:
                raise RuntimeError("Failed to store retrained model in ClearML; aborting to enforce ClearML-only storage")
            model_info["clearml_task_id"] = task_id
            model_info["stored_in_clearml"] = True
        if model_id in self.model_metadata:
                ...
                for task in tasks:
                    try:
                        if artifact_name in task.artifacts:
                            ...
                            logger.info(f"Model {model_id} deleted from ClearML (task: {task.id})")
                            break
                    except Exception:
                        continue
            except Exception as e:
                logger.warning(f"Could not delete model from ClearML: {str(e)}", exc_info=True)
    ...
    def get_info(self, model_id: str) -> Dict:
        ...
        if model_id in self.model_metadata:
            return {
                ...
                "status": "registered",
                "stored_in_clearml": self.model_metadata[model_id].get("stored_in_clearml", False),
        ...
        if clearml.enabled:
            ...
            try:
                ...
                for task in tasks:
                    artifact_name = f"model_{model_id}"
                    if artifact_name in task.artifacts:
                        logger.info(f"Found model {model_id} in ClearML (task: {task.id})")
                        return {
                            "model_id": model_id,
                ...
                logger.error(f"Error checking ClearML for model info: {str(e)}", exc_info=True)
    ...
    def _load_model(self, model_id: str):
        ...
        clearml = get_clearml_manager()
        if clearml.enabled:
            logger.info(f"Model {model_id} not in memory, attempting to load from ClearML...")
            try:
                artifact_name = f"model_{model_id}"
                tasks = Task.get_tasks(project_name="ML-OPS-HW1")
                try:
                    MODEL_LOAD_TIMEOUT = float(os.environ.get("MODEL_LOAD_TIMEOUT", "10"))
                except Exception:
                ...
                for task in tasks:
                    try:
                        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as exe:
                            fut = exe.submit(lambda a=task, name=artifact_name: a.artifacts[name].get())
                            try:
                                model = fut.result(timeout=MODEL_LOAD_TIMEOUT)
                            except concurrent.futures.TimeoutError:
                                logger.warning(f"Timeout loading model {model_id} from task {task.id}")
                                continue
                            except Exception as e:
                                logger.debug(f"Artifact get exception for model {model_id} task {task.id}: {e}")
                                continue

                        if model is not None:
                            logger.info(f"Model {model_id} loaded from ClearML (task: {task.id})")
                            return model
                ...
                logger.error(f"Error loading model from ClearML: {str(e)}", exc_info=True)
    ...
    def _log_to_clearml(self, model_id: str, model_type: str, hyperparams: Dict,
                       metrics: Dict, task_suffix: str = "train", model=None) -> Optional[str]:
        """Log model training to ClearML and save model object."""
        clearml = get_clearml_manager()
        if not clearml.enabled:
            logger.info(f"ClearML disabled, model {model_id} trained but not stored in ClearML")
            return None
        ...
        try:
            ...
            for metric_name, metric_value in metrics.items():
                clearml.log_scalar(title="metrics", series=metric_name, value=metric_value)
            
            if model:
                artifact_name = f"model_{model_id}"
                clearml.upload_model_object(model, artifact_name)
                logger.info(f"Model {model_id} saved to ClearML as primary storage: {artifact_name}")
            
            clearml.close()
            return task_id
        except Exception as e:
            logger.error(f"ClearML logging error: {str(e)}", exc_info=True)
            logger.warning(f"Model {model_id} trained successfully but not saved to ClearML")
            return None
